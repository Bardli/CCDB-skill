...
If `$CC_CLUSTER` is unset you are probably not on an Alliance node, or site-config
has not yet sourced — open a fresh login shell.
...
| **Cluster-specific facts** (GPUs, partitions, login host, quirks) | `references/clusters/$CC_CLUSTER.md` |
...
- `$CC_CLUSTER`, `$SCRATCH`, `$HOME`, `$PROJECT` are set on every Alliance node. Use them in scripts instead of hard-coded paths so the same script works on any cluster.
